...
const TEST_CASES = [
  ...
  // Proteins
  { item:'chicken breast',    rawAmount:'1 piece',  expectedGrams:174, expectedCal:184, expectedCarbs:0,   expectedProtein:39, expectedFat:3,  tolerance:0.25 },
...
function toGrams(qty, unit, itemName) {
  ...
  if (countableUnits.has(u)||u==='') {
    for (const [k,v] of Object.entries(COUNTABLE).sort((a,b)=>b[0].length-a[0].length)) { if (name.includes(k)) return {grams:qty*v,qty,unit:u}; }
...
function estimateGrams(name,qty,unit) {
  ...
  for (const [k,v] of Object.entries(COUNTABLE).sort((a,b)=>b[0].length-a[0].length)) { if (n.includes(k)||combined.includes(k)||nwu.includes(k)) return qty*v; }
